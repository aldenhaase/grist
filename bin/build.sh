...
go run /workspace/bin/construct_yaml.go test.yaml
python2 /workspace/bin/runAPI.py --application=lystr-354722 /workspace/src/server/go-app/app.yaml --support_datastore_emulator=yes --port=8081
if [ "$?" -ne "0" ]; then
